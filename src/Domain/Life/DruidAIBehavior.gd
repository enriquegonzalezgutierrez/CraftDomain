...
#              Planning (GOAP) for the Forest Druid NPC with smart wall navigation.
...
class DruidPatrolAction extends GOAPAction:
	...
	func execute_step(bb: AIBlackboard, delta: float) -> bool:
		...
		VoxelKinematicService.apply_motion_vectors(host, ai, wander_dir, DruidAIBehavior.SPEED_PATROL)
		return false
