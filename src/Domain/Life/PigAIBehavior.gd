...
#              Planning (GOAP) for the Wild Pig with smart wall navigation.
...
class PigWanderAction extends GOAPAction:
	...
	func execute_step(bb: AIBlackboard, delta: float) -> bool:
		...
		VoxelKinematicService.apply_motion_vectors(host, ai, wander_dir, PigAIBehavior.SPEED_WALK)
		return false
