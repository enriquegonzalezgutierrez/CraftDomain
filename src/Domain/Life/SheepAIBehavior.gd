# ==============================================================================
# Pathfile: res://src/Domain/Life/SheepAIBehavior.gd
# Description: Pure Domain AI behavior strategy implementing specialized 
#              grazing, digestion, and wool-growth routines for the Fluffy Sheep.
# SOLID COMPLIANCE:
# - Single Responsibility Principle (SRP): Coordinates strictly sheep state 
#   transitions, grazing checks, and wool regrowth conditions.
# - Layered DDD Compliance: Pure logical state calculations with zero framework 
#   leakage, keeping scale and particle implementations in Infrastructure.
# Author: Enrique González Gutiérrez
# Email: enrique.gonzalez.gutierrez@gmail.com
# ==============================================================================
class_name SheepAIBehavior
extends IAIBehavior

const SPEED_WALK: float = 0.7
const SPEED_PANIC: float = 2.4
const SENSORY_RANGE_SQ: float = 64.0 # 8.0m threat detection radius

# Grazing duration parameters
const GRAZE_INTERVAL_MIN_SEC: float = 12.0
const GRAZE_INTERVAL_MAX_SEC: float = 24.0
...
# Specialized Sheep States: 0 = WANDERING, 1 = GRAZING
const STATE_WANDERING = 0
const STATE_GRAZING = 1

# Decoupled metadata keys
const META_STATE := "sheep_local_state"
const META_WANDER_TIMER := "sheep_wander_timer"
const META_WANDER_DIR := "sheep_wander_dir"
const META_GRAZE_TIMER := "sheep_graze_timer"
const META_GRAZE_COOLDOWN := "sheep_graze_cooldown"
const META_IS_FLUFFY := "sheep_is_fluffy"
...
## Concrete Contract: Drives grazing, soil-munching, and wool regrowth cycles
...
func _process_panic_state(host: Object, delta: float) -> void:
	...
	if wander_timer <= 0.0:
		wander_timer = randf_range(0.4, 0.8) # High frequency direction changes
		var angle := randf() * TAU
...
func _calculate_next_wander_step(host: Object) -> void:
	...
	var host_pos: Vector3 = host.get("global_position")
	var cooldown: float = host.get_meta(META_GRAZE_COOLDOWN) as float
	var is_fluffy: bool = host.get_meta(META_IS_FLUFFY) as bool
	
	# Shaved/Sheared sheep actively search for Grass to digest and regrow wool
	if ws != null and not is_fluffy and cooldown <= 0.0 and randf() < 0.50:
...
func _complete_soil_grazing(host: Object) -> void:
	...
	if ws != null and parent.has_method("set_block_globally"):
		...
		# Turns Grass (3) into Dirt (2) and regrows wool coat (is_fluffy = true)
	...
	# Restart grazing cooldown (12s to 24s)
...
func _initialize_metadata_if_missing(host: Object) -> void:
	...
	if not host.has_meta(META_GRAZE_COOLDOWN): host.set_meta(META_GRAZE_COOLDOWN, 6.0) # Grace period on spawn
	if not host.has_meta(META_IS_FLUFFY): host.set_meta(META_IS_FLUFFY, true) # Spawns maximum fluffy
