...
#              grazing, digestion, predator evasion, and wheat-luring.
# SOLID COMPLIANCE:
...
# Grazing parameters
const GRAZE_INTERVAL_MIN: float = 12.0
const GRAZE_INTERVAL_MAX: float = 24.0
...
func _process_predator_evasion(host: Object, ai: Object, delta: float) -> bool:
	...
	wander_timer -= delta
	if wander_timer <= 0.0:
		wander_timer = randf_range(0.4, 0.8)
		...
		escape_dir.y = 0.0
		escape_dir = escape_dir.rotated(Vector3.UP, randf_range(-0.4, 0.4))
...
func _process_food_luring(host: Object, ai: Object) -> bool:
	...
	var speed := SPEED_FOLLOW if diff.length_squared() > 2.25 else 0.0
...
func _calculate_next_wander_step(host: Object) -> void:
	...
	# Shaved sheep actively search for Grass to digest and regrow wool
...
func _initialize_metadata_if_missing(host: Object) -> void:
	...
	if not host.has_meta(META_IS_FLUFFY): host.set_meta(META_IS_FLUFFY, true)
