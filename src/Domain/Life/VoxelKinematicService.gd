...
static func _is_path_invalidated_by_voxel_changes(host: CharacterBody3D, target_coord: Vector3i, meta_index_key: String) -> bool:
...
static func _is_node_still_walkable(ws: WorldState, coord: Vector3i) -> bool:
	...
	if BlockLibrary.is_solid(block_feet):
		return false 
