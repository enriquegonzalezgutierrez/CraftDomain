...
#              Planning (GOAP) for the Forest Red Fox with smart wall navigation.
...
const SPEED_SNEAK: float = 1.4
const SPEED_POUNCE: float = 5.2
...
class FoxPatrolAction extends GOAPAction:
	...
	func execute_step(bb: AIBlackboard, delta: float) -> bool:
		...
		VoxelKinematicService.apply_motion_vectors(host, ai, wander_dir, FoxAIBehavior.SPEED_PATROL)
		return false
