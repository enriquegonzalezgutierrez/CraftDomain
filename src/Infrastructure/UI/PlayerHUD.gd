...
func toggle_crafting_workshop(p_visible: bool) -> void:
	if (_pause_widget and _pause_widget.visible) or is_instance_valid(_inventory_overlay):
		return 
...
func toggle_inventory_backpack(p_visible: bool) -> void:
	if (_pause_widget and _pause_widget.visible) or is_instance_valid(_crafting_overlay):
		return 
...
func show_quest_notification(header: String, quest_title: String) -> void:
	...
	header_lbl.text = "🏆 " + tr(header).to_upper() # Localized Toast Title
	...
	desc_lbl.text = tr(quest_title) # Localized Quest text
