...
# Description: Physics controller for the cave Miner NPC, utilizing the 
#              Strategy pattern to load dynamic animations cleanly.
#              SOLID COMPLIANCE:
#              - Single Responsibility Principle (SRP): Handles exclusively miner
#                dialogues and player tracking, delegating rendering to the Strategy.
...
func _get_humanoid_role() -> int:
	return ProceduralVoxelRepresentation.RoleType.MINER
...
func _select_procedural_greeting_key() -> String:
	...
	if is_night: return "DIALOGUE_MINER_NIGHT"
