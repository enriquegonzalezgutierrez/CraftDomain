...
## Decoupled local metadata key to prevent cyclic compile locks with SheepAIBehavior
...
func _spawn_wool_shear_particles() -> void:
	var particles := CPUParticles3D.new()
	_configure_shear_particle_properties(particles)
	
	var mesh := BoxMesh.new()
	mesh.size = Vector3(0.12, 0.12, 0.12)
	var mat := StandardMaterial3D.new()
	mat.albedo_color = Color(0.92, 0.92, 0.95, 0.9) 
	mat.shading_mode = BaseMaterial3D.SHADING_MODE_UNSHADED
...
func _configure_shear_particle_properties(particles: CPUParticles3D) -> void:
	particles.amount = 16
	particles.one_shot = true
