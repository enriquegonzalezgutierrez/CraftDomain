...
## Configures and attaches all required floating UI nodes to the parent host
...
## Updates visibility, texts, and colors of UI layers based on quest status
...
## Safely cleans up instantiated UI nodes to prevent memory leaks on death
func cleanup() -> void:
...
func _setup_nameplate() -> void:
	if not is_instance_valid(host) or is_instance_valid(_nameplate):
		return
...
func _setup_quest_arrow() -> void:
	if not is_instance_valid(host) or is_instance_valid(_quest_arrow):
		return
...
func _get_task_subtitle() -> String:
	...
	return "\n[ " + tr("SHOWCASE_TASK_" + lookup_key).to_upper() + " ]"


func _apply_uniform_ui_scaling(node: Node3D) -> void:
	if not is_instance_valid(host) or not is_instance_valid(node):
		return
