...
func _build_visual_representation() -> void:
	...
	# 1. Base Legs & Metallic Chrome Feet (Attached to the bouncing joint)
	...
	visual_component.create_box(visual_component.body_bob_node, Vector3(0.12, 0.12, 0.05), Vector3(0, 0.65, -0.23), neon_cyan)
	...
	visual_component.create_box(visual_component.head_node, Vector3(0.35, 0.37, 0.35), Vector3(0, 0.185, 0), steel_color) # Face
	
	# Sleek glowing cyan horizontal visor plate (Covers standard blinking eyes, no eyes tracked here)
	visual_component.create_box(visual_component.head_node, Vector3(0.38, 0.10, 0.08), Vector3(0, 0.19, -0.16), neon_cyan)
	# Top head-antenna
	visual_component.create_box(visual_component.head_node, Vector3(0.04, 0.18, 0.04), Vector3(0, 0.46, 0), chrome_color)
	visual_component.create_box(visual_component.head_node, Vector3(0.06, 0.06, 0.06), Vector3(0, 0.54, 0), neon_magenta) # Antenna tip
...
func interact(player_node: CharacterBody3D) -> void:
	...
	if is_instance_valid(hud):
		...
		hud.open_dialogue(intro_node, "Android", self)
