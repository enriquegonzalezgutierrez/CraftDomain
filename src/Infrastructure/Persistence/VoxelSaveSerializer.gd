...
# SOLID COMPLIANCE:
# - Single Responsibility Principle (SRP): Handles exclusively the packing and 
#   unpacking of in-memory data types (Vectors, Array lists, and Enums) into 
#   JSON-compatible dictionary structures, isolating formatting from disk I/O.
# - Open-Closed Principle (OCP): Easily extendable with new data structures 
#   (e.g., adding player level, health, or active skills) without modifying I/O scripts.
# Author: Enrique González Gutiérrez <enrique.gonzalez.gutierrez@gmail.com>
# File: res://src/Infrastructure/Persistence/VoxelSaveSerializer.gd
# ==============================================================================
...
## Translates Vector3i coordinates into "x,y,z" string keys.
static func serialize_chunk_deltas(modifications: Dictionary) -> Dictionary:
	var serialized: Dictionary = {}
	
	# FIX: Explicit static typing on loop coordinate keys
...
static func deserialize_chunk_deltas(serialized_data: Dictionary) -> Dictionary:
	...
	# FIX: Explicit static typing on string keys
...
## Unpacks and safely parses raw JSON dictionary variants back into typed Vector3 and state values.
static func deserialize_global_state(data: Dictionary) -> Dictionary:
	...
	# Safely unpack 3D coordinates target
	if data.has("player_pos"):
		var p_pos: Dictionary = data["player_pos"] as Dictionary
		state["player_pos"] = Vector3(
			float(p_pos.get("x", 8.5)), 
			float(p_pos.get("y", 14.0)), 
			float(p_pos.get("z", 8.5))
		)
	else:
		state["player_pos"] = Vector3(8.5, 14.0, 8.5) # Default spawn fallback
		
	# Safely unpack camera rotation targets
	if data.has("player_rot"):
		var p_rot: Dictionary = data["player_rot"] as Dictionary
		state["player_rot"] = Vector3(
			float(p_rot.get("x", 0.0)), 
			float(p_rot.get("y", 0.0)), 
			float(p_rot.get("z", 0.0))
		)
	else:
		state["player_rot"] = Vector3.ZERO
