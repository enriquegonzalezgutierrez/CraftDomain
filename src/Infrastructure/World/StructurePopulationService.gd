...
# ==============================================================================
# GRAND CASTLE SPAWN POINTS (SRP Decomposed & Well Spaced)
# ==============================================================================
...
static func _populate_castle_gates() -> void:
	...
	_population_registry[c_12_0_12] = [
		PopulationPoint.new(false, 102, Vector3(194.5, 13.0, 224.5)), # Guard Left
		PopulationPoint.new(false, 102, Vector3(205.5, 13.0, 224.5)), # Guard Right
...
static func _populate_castle_courtyard() -> void:
	...
	_population_registry[c_12_0_11] = [
		PopulationPoint.new(false, 102, Vector3(192.5, 13.0, 188.5)), # West Guard Post
		PopulationPoint.new(false, 102, Vector3(207.5, 13.0, 188.5)), # East Guard Post
		PopulationPoint.new(false, 100, Vector3(196.5, 13.0, 192.5)), # Villager Plaza
		PopulationPoint.new(false, 110, Vector3(200.5, 13.0, 196.5))  # Quique Hallway
	]
...
static func _populate_castle_keep() -> void:
	...
	_population_registry[c_11_0_11] = [
		PopulationPoint.new(false, 102, Vector3(188.5, 13.0, 194.5))  # Keep Guard
	]
	
	var c_13_1_11 := Vector3i(13, 1, 11)
	_population_registry[c_13_1_11] = [
		PopulationPoint.new(false, 102, Vector3(208.5, 19.5, 188.5)), # Upper Guard
...
static func _populate_harbor_spawns() -> void:
	...
	_population_registry[c_neg9_0_0] = [
		PopulationPoint.new(false, 100, Vector3(-138.5, 12.0, 8.5)),  # Villager Dock
		PopulationPoint.new(false, 101, Vector3(-132.5, 12.0, -8.5)), # Merchant Stall
		PopulationPoint.new(false, 102, Vector3(-128.5, 12.5, -2.5))  # Guard Outpost
	]
	
	var c_neg10_1_0 := Vector3i(-10, 1, 0)
	_population_registry[c_neg10_1_0] = [
		PopulationPoint.new(false, 102, Vector3(-150.5, 17.5, 0.5)),  # Guard Ship Deck
		PopulationPoint.new(true, 200, Vector3(-146.5, 17.5, -2.5)),  # Chest
		PopulationPoint.new(false, 100, Vector3(-162.5, 7.5, -3.5))   # Villager Cabin
	]
...
static func _populate_nether_spawns() -> void:
	...
	_population_registry[c_neg19_0_neg19] = [
		PopulationPoint.new(false, 10, Vector3(-292.5, 9.5, -298.5)),  # Zombie
		PopulationPoint.new(false, 10, Vector3(-308.5, 9.5, -298.5))   # Zombie
	]
...
static func _populate_cabin_spawns() -> void:
	...
	_population_registry[c_18_0_neg19] = [
		...
		PopulationPoint.new(false, 103, Vector3(290.5, 11.0, -288.5)), # Farmer Wheat
		PopulationPoint.new(false, 107, Vector3(298.5, 11.0, -305.5))  # Golem Gate
	]
...
static func _populate_emerald_loop_spawns() -> void:
	...
	_population_registry[c_18_0_18] = [
		...
		PopulationPoint.new(false, 14, Vector3(294.5, 13.0, 292.5)),  # Badnik Crab
